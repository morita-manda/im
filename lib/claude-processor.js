...
function emptyPeriod() {
  return {
    ...
    sga: { personnel: "", executivePay: "", salary: "", socialInsurance: "", rent: "",
           advertising: "", depreciation: "", other: "" },
  };
}
...

{
  ...
  "sga": {
    ...
    "depreciation": "減価償却費", "other": "その他経費合計"
  }
}
...
希望譲渡金額が未入力です。抽出済み財務データをもとに、以下の手法で試算し transfer.price に記載してください。
- 年買法：修正純資産 ＋ 経常利益（または営業利益）× 3〜5年分
- EBITDAマルチプル：EBITDA × 3〜5倍（中小企業の目安）
利用可能なデータで計算し、「約○○百万円〜○○百万円（試算値）」の形式のみで記載してください。計算過程は含めないでください。
...
`.trim();
  ...
  // 代表者名が意味のない値の場合は空文字にクリア
  const unusableRep = ["不明", "不明確", "N/A", "-", "未記入", "不詳", "未取得", "HPまたはPDFから取得してください"];
  if (data.company && unusableRep.some(v => (data.company.rep || "").trim() === v)) {
    data.company.rep = "";
  }
  ...
  if (!userInput.price && (!data.transfer || !data.transfer.price)) {
