...
【単位の正規化（重要）】
- 金額はすべて「円（実額）」に統一して出力すること。書類記載の単位に応じて以下の通り換算する:
    - 単位:円   → そのまま
    - 単位:千円  → ×1,000
    - 単位:万円  → ×10,000
    - 単位:百万円 → ×1,000,000
    - 単位:億円  → ×100,000,000
- 例) 「（単位：千円）売上高 150,000」→ revenue は "150000000"（円）。
- 例) 「売上高 1.5億円」→ revenue は "150000000"（円）。
- 検算: 出力した売上高の桁感が決算規模と整合するか必ず確認する（年商1.5億円なら revenue は 150000000=9桁）。単位の取り違えに注意。
- grossMargin / operatingMargin / ordinaryMargin などの「率（%）」は金額ではないので換算せず "95.0%" の形式のまま出力する。

数値は円単位の文字列（カンマ無しの整数）で。率（%）は "○○%" 形式。読み取れない項目は空文字。JSONのみを返してください。`;
...
async function processWithClaude({ pdfFilePaths, pdfTexts, webInfo, userInput, shareholders, employeeBreakdown }) {
  ...
  // rawCompanyDetailsをテキスト形式に変換（プロンプトへ埋め込み用）
  const rawDetailsText = (webInfo.rawCompanyDetails && webInfo.rawCompanyDetails.length > 0)
    ? webInfo.rawCompanyDetails.map((d) => `${d.label}: ${d.value}`).join("\n")
    : "（取得できず）";
...
`.trim();
  ...
  // rawCompanyDetailsがあれば直接使用（HTMLから確実に取得済み）
  const raw = webInfo.rawCompanyDetails || [];
  if (raw.length > 0) {
    data.companyDetails = raw;

    // 住所が空なら rawCompanyDetails から補完
    if (!data.company.address) {
      const item = raw.find((d) => /住所|所在地|本社/.test(d.label));
      if (item) data.company.address = item.value;
    }

    // 代表者が空なら rawCompanyDetails から補完
    if (!data.company.rep) {
      const item = raw.find((d) => /代表|社長|会長|取締役|理事長|役員|CEO/i.test(d.label));
      if (item) {
        const val = item.value.trim().replace(/ほか.*$|など.*$/, "").trim();
        data.company.rep = /代表|取締役|社長|会長|理事長|CEO/i.test(val)
          ? val
          : `${item.label} ${val}`;
      }
    }
  ...
  // スクレイパーが正規表現で直接抽出した値で補完（構造化データより最終フォールバック）
  if (!data.company.address && webInfo.extractedAddress) {
    data.company.address = webInfo.extractedAddress;
  }
  if (!data.company.rep && webInfo.extractedRep) {
    data.company.rep = webInfo.extractedRep;
  }
  console.log(`[Claude] 最終: address="${data.company.address}", rep="${data.company.rep}", details=${(data.companyDetails||[]).length}件`);
