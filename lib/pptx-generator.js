...
// ========== 金額フォーマット（円ベース → 億円/万円 自動表示）==========
// 抽出側（claude-processor.js）が金額を「円（実額）」で出力する前提で、
// 表示単位を自動選択する。1億円以上は「○○億円」、未満は「○○万円」。
// 既に単位付き（○円・○%）や数値でない文字列はそのまま返す（多重変換を防ぐ）。
function fmtJpy(val) {
  if (val == null || val === "" || val === "-") return val || "-";
  const s = String(val).trim();
  if (/[%円]/.test(s)) return s;                       // 既に単位付き or 率 → そのまま
  const n = parseFloat(s.replace(/[,，\s]/g, ""));
  if (isNaN(n)) return val;                             // 数値でない → そのまま
  const sign = n < 0 ? "-" : "";
  const abs = Math.abs(n);
  if (abs < 1e4) return sign + Math.round(abs) + "円";  // 1万円未満は円表示
  if (abs >= 1e8) {                                      // 1億円以上 → 億円
    const oku = Math.round((abs / 1e8) * 100) / 100;
    return sign + oku.toString() + "億円";
  }
  const man = Math.round(abs / 1e4);                     // 万円
  return sign + man.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "万円";
}

// 期データ（pl/bs/sga）の金額フィールドを表示用文字列へ正規化する。
// 率（%）フィールドは対象外。べき等（既に変換済みなら何もしない）。
const _MONEY_KEYS = {
  pl: ["revenue", "cogs", "grossProfit", "sga", "operatingProfit", "nonOpIncome",
       "nonOpExpense", "ordinaryProfit", "tax", "netProfit", "ebitda", "depreciation"],
...
function normalizeFinancialUnits(period) {
  if (!period || typeof period !== "object") return period;
  for (const group of Object.keys(_MONEY_KEYS)) {
    const obj = period[group];
    if (!obj || typeof obj !== "object") continue;
    for (const key of _MONEY_KEYS[group]) {
      if (obj[key] != null && obj[key] !== "") obj[key] = fmtJpy(obj[key]);
    }
  }
  return period;
}
...
async function generateIM(data) {
  ...
  // 金額表示は円ベース → 億円/万円 自動表示（fmtJpy）に統一
  const toMan = fmtJpy;

  // 抽出データ（円ベース）の金額フィールドを表示用文字列へ正規化。
  // 財務諸表テーブル・KPIカード・多期比較すべてがこの正規化済みデータを参照する。
  normalizeFinancialUnits(data.financials);
  if (Array.isArray(data.financialPeriods)) data.financialPeriods.forEach(normalizeFinancialUnits);
  ...
  // ========== Slide 1: 表紙 ==========
  {
    ...
    s.addText((c.name || "会社名").replace(/[\(（][^\)）]*[\)）]/g, "").trim(), {
  ...
  // ========== Slide 5: 会社概要サマリー ==========
  {
    ...
    companyRows.forEach((row) => {
      const rh = row.label === "所在地" ? 0.6 : 0.44;
      ...
      s.addText(row.label, { x: 0.55, y: companyY + 0.05, w: 1.4, h: rh - 0.1, fontSize: 12.5, bold: true, color: DGRAY, fontFace: "メイリオ", valign: "middle" });
      s.addText(row.value, { x: 2.05, y: companyY + 0.05, w: 4.2, h: rh - 0.1, fontSize: 12.5, color: BLACK, fontFace: "メイリオ", valign: "top", shrinkText: true });
      companyY += rh + 0.02;
    });
  ...
  // ========== Slide 8: 対象会社概要 ==========
  {
    ...
    displayStrengths.slice(0, 6).forEach((item) => {
      const itemH = 0.95;
      s.addShape("rect", { x: 7.0, y: yR, w: 1.6, h: itemH, fill: { color: GREEN_BG }, line: { color: MGRAY } });
      s.addShape("rect", { x: 8.6, y: yR, w: 4.4, h: itemH, fill: { color: WHITE }, line: { color: MGRAY } });
      s.addText(item.label || "", {
        x: 7.05, y: yR + 0.06, w: 1.5, h: itemH - 0.12, fontSize: 11, bold: true, color: GREEN, align: "center", fontFace: "メイリオ", valign: "middle",
      });
      s.addText(item.desc || "", {
        x: 8.65, y: yR + 0.05, w: 4.3, h: itemH - 0.1, fontSize: 12, color: BLACK, fontFace: "メイリオ", valign: "top", shrinkText: true,
      });
      yR += itemH + 0.02;
    });
  }
