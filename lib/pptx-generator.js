...
async function generateIM(data) {
  ...
  // ========== Slide 5: 会社概要サマリー ==========
  {
    ...
    companyRows.forEach((row, i) => {
      ...
      s.addText(row[1], { x: 2.05, y: companyY + 0.05, w: 4.2, h: rh - 0.1, fontSize: 12.5, color: BLACK, fontFace: "メイリオ", valign: "top", wrap: true });
      companyY += rh + 0.02;
    });
    ...
    if (screenshotPath) {
      s.addImage({ path: screenshotPath, x: 6.9, y: 2.15, w: 6.0, h: 4.1 });
    } else {
  ...
  // ========== Slide 6: 主要財務指標 ==========
  {
    ...
    // 上段：4つの主要指標カード
    const topKpis = [
      { label: "売上高",    value: plLargest.revenue         || "-" },
      { label: "営業利益",  value: plLargest.operatingProfit || "-", sub: plLargest.operatingMargin ? `利益率 ${plLargest.operatingMargin}` : "" },
      { label: "経常利益",  value: plLargest.ordinaryProfit  || "-", sub: plLargest.ordinaryMargin  ? `利益率 ${plLargest.ordinaryMargin}`  : "" },
      { label: "当期純利益",value: plLargest.netProfit        || "-" },
    ];
  ...
  // ========== Slide 8: 対象会社概要 ==========
  {
    ...
    function detailRowHeight(label) {
      if (label === "事業内容" || label === "事業目的") return 1.4;
      if (label === "本社住所" || label === "所在地" || label === "設立" || label === "沿革") return 0.6;
      return 0.44;
    }
    let detailY = 1.3;
    companyDetails.forEach((row) => {
      ...
      s.addText(row.value, { x: 2.15, y: detailY + 0.05, w: 4.1, h: rh - 0.1, fontSize: 12, color: BLACK, fontFace: "メイリオ", valign: "top", wrap: true });
      detailY += rh + 0.02;
    });
  ...
  // ========== 実績データ ==========
  {
    ...
    const topKpiItems = hasKpi
      ? [
          ...
          { label: kpi.procedures ? "累計施術件数"  : "営業利益",  value: kpi.procedures || pl.operatingProfit  || "-", note: "" },
          { label: kpi.reviewCount? "口コミ件数"    : "経常利益",  value: kpi.reviewCount|| pl.ordinaryProfit   || "-", note: kpi.reviewCount ? "口コミプラットフォーム" : "" },
      ...
      : [
          ...
          { label: "営業利益",  value: pl.operatingProfit  || "-", note: pl.operatingMargin ? `利益率 ${pl.operatingMargin}` : "" },
          { label: "経常利益",  value: pl.ordinaryProfit   || "-", note: pl.ordinaryMargin  ? `利益率 ${pl.ordinaryMargin}`  : "" },
    ...
    const finCandidates = [
      { label: "粗利率",       value: pl.grossMargin    || "-" },
      { label: "EBITDA（概算）",value: pl.ebitda        || "-" },
      { label: "販管費",       value: pl.sga            || "-" },
      { label: "純資産",       value: bs.equity         || "-" },
      { label: "売上高",       value: pl.revenue        || "-" },
      { label: "当期純利益",   value: pl.netProfit      || "-" },
      { label: "経常利益",     value: pl.ordinaryProfit || "-" },
    ];
