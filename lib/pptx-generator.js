...
async function generateIM(data) {
  ...
  // ========== 実績データ ==========
  {
    ...
    const topKpiItems = hasKpi
      ? [
          { label: kpi.customers  ? "累計顧客数"    : "売上高",    value: kpi.customers  || plLargest.revenue         || "-", note: "" },
          { label: kpi.procedures ? "累計施術件数"  : "営業利益",  value: kpi.procedures || plLargest.operatingProfit  || "-", note: "" },
          { label: kpi.reviewCount? "口コミ件数"    : "経常利益",  value: kpi.reviewCount|| plLargest.ordinaryProfit   || "-", note: kpi.reviewCount ? "口コミプラットフォーム" : "" },
          { label: kpi.reviewScore? "平均評価"      : "当期純利益",value: kpi.reviewScore|| plLargest.netProfit        || "-", note: kpi.reviewScore ? "（5点満点）" : "" },
        ]
      : [
          { label: "売上高",    value: plLargest.revenue          || "-", note: "" },
          { label: "営業利益",  value: plLargest.operatingProfit  || "-", note: plLargest.operatingMargin ? `利益率 ${plLargest.operatingMargin}` : "" },
          { label: "経常利益",  value: plLargest.ordinaryProfit   || "-", note: plLargest.ordinaryMargin  ? `利益率 ${plLargest.ordinaryMargin}`  : "" },
          { label: "当期純利益",value: plLargest.netProfit         || "-", note: "" },
        ];
    ...
    const finCandidates = [
      ...
      { label: "EBITDA（概算）",value: plLargest.ebitda        || "-" },
      { label: "販管費",       value: plLargest.sga            || "-" },
      { label: "純資産",       value: bsLargest.equity         || "-" },
      { label: "売上高",       value: plLargest.revenue        || "-" },
      { label: "当期純利益",   value: plLargest.netProfit      || "-" },
      { label: "経常利益",     value: plLargest.ordinaryProfit || "-" },
    ];
